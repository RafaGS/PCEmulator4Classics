...
// FabGL custom BIOS ROM: provides all functional INT handlers via soft-traps.
static const uint8_t biosrom[] = {
  #include BIOS_ROM_HEADER
};

#if BIOS_HAS_ROM_BASIC && defined(BIOS_ROM_BASIC_EMBEDDED_HEADER)
static const uint8_t bios_rom_basic[] = {
  #include BIOS_ROM_BASIC_EMBEDDED_HEADER
};
#endif

#ifdef BIOS_IDENTITY_HEADER
// Platform identity ROM: loaded as read-only overlay at BIOS_ROM_LOAD_ADDR.
// Provides IBM-compatible identity bytes (date, machine ID, copyright, checksum)
// so that diagnostic tools see the correct machine.  Not executed directly.
static const uint8_t bios_identity_rom[] = {
  #include BIOS_IDENTITY_HEADER
};
#endif
...
void BIOS::init(Machine * machine)
{
  ...
  // copy custom FabGL BIOS (handles all INTs via soft-traps)
  memcpy(m_memory + BIOS_ADDR, biosrom, sizeof(biosrom));

#ifdef BIOS_IDENTITY_HEADER
  // Overlay platform identity ROM at BIOS_ROM_LOAD_ADDR (e.g. 0xFE000 for IBM PC 5150).
  // This fills F000:E000-FFFF with the real IBM identity bytes without executing that ROM.
...
void BIOS::applyPlatformIdentity()
{
  // Keep BIOS date/model/checksum aligned with selected profile.
  memcpy(m_memory + 0xFFFF5, BIOS_OEM_DATE, 8);
  m_memory[0xFFFFE] = BIOS_MACHINE_ID & 0xFF;
  m_memory[0xFFFFF] = BIOS_ROM_CHECKSUM & 0xFF;

  // BDA equipment word (INT 11h) and conventional memory in KB (INT 12h / [0x413]).
  m_memory[BIOS_DATAAREA_ADDR + 0x10] = BIOS_EQUIPMENT_WORD & 0xFF;
  m_memory[BIOS_DATAAREA_ADDR + 0x11] = (BIOS_EQUIPMENT_WORD >> 8) & 0xFF;
  uint16_t convram = BIOS_CONVENTIONAL_RAM_KB;
  m_memory[0x413] = convram & 0xFF;
  m_memory[0x414] = (convram >> 8) & 0xFF;

  // INT 12h in bios.asm returns [CS:memsize] (ROM variable), not only BDA[0x413].
  // Locate BIOS data template in F000 ROM once and patch its memsize field too.
  static uint32_t biosDataAddr = 0;
  if (biosDataAddr == 0) {
    // bios_data starts with COM/LPT defaults: F8 03 F8 02 00 00 00 00 78 03 ...
    static const uint8_t biosDataSig[] = {
      0xF8, 0x03, 0xF8, 0x02, 0x00, 0x00, 0x00, 0x00,
      0x78, 0x03, 0x00, 0x00, 0x00, 0x00
    };
    for (uint32_t p = BIOS_ADDR; p < 0x100000 - sizeof(biosDataSig); ++p) {
      if (memcmp(m_memory + p, biosDataSig, sizeof(biosDataSig)) == 0) {
        biosDataAddr = p;
        break;
      }
    }
  ...
  if (biosDataAddr) {
    // memsize is at bios_data + 0x13
    m_memory[biosDataAddr + 0x13] = convram & 0xFF;
    m_memory[biosDataAddr + 0x14] = (convram >> 8) & 0xFF;
  }

#if BIOS_HAS_ROM_BASIC
  // POST can rewrite IVT entries from its internal template.
  // Keep INT 18h pointing to ROM BASIC (F600:0607 on IBM 5150).
  m_memory[0x0060] = BIOS_ROM_BASIC_OFF & 0xFF;
  m_memory[0x0061] = (BIOS_ROM_BASIC_OFF >> 8) & 0xFF;
  m_memory[0x0062] = BIOS_ROM_BASIC_SEG & 0xFF;
  m_memory[0x0063] = (BIOS_ROM_BASIC_SEG >> 8) & 0xFF;
#endif
}


void BIOS::loadRomBasic()
{
#if BIOS_HAS_ROM_BASIC
  // Prefer embedded ROM BASIC image when available.
#if defined(BIOS_ROM_BASIC_EMBEDDED_HEADER)
  memcpy(m_memory + BIOS_ROM_BASIC_ADDR, bios_rom_basic, BIOS_ROM_BASIC_SIZE);
#else
  // Optional SD fallback: load the combined ROM BASIC image from SD.
  auto f = FileBrowser(SD_MOUNT_PATH).openFile(BIOS_ROM_BASIC_FILE, "rb");
  if (f) {
    fread(m_memory + BIOS_ROM_BASIC_ADDR, 1, BIOS_ROM_BASIC_SIZE, f);
    fclose(f);
  }
#endif
  // Point INT 18h vector to the ROM BASIC entry (IVT lives at physical 0:0x00–0x3FF).
  m_memory[0x0060] = BIOS_ROM_BASIC_OFF & 0xFF;
  m_memory[0x0061] = (BIOS_ROM_BASIC_OFF >> 8) & 0xFF;
  m_memory[0x0062] = BIOS_ROM_BASIC_SEG & 0xFF;
  m_memory[0x0063] = (BIOS_ROM_BASIC_SEG >> 8) & 0xFF;
...
{
  // BIOS POST may rewrite mem_top and BDA values; enforce selected profile continuously.
  applyPlatformIdentity();
...
void BIOS::getKeyFromBuffer()
{
  ...
  if (*head != *tail) {
    ...
    if (!filtered) {
      ...
      i8086::setFlagZF(0);
      // Return key exactly as BIOS expects: AL = ASCII, AH = scan code.
      // Using explicit byte setters avoids any potential AX byte-order mismatch.
      i8086::setAL(k & 0xFF);
      i8086::setAH((k >> 8) & 0xFF);
    }
