...
void Machine::runTask(void * pvParameters)
{
...
#if EMU_CPU_FREQ_HZ > 0
  // use high resolution timer (microseconds) to schedule CPU steps
  uint64_t lastUs = esp_timer_get_time();
  uint64_t stepBudgetNum = 0;
  constexpr uint64_t STEP_BUDGET_DEN = EMU_CPU_CYCLES_PER_STEP_NUM * 1000000ULL;
  // Keep each loop bounded to avoid long catch-up bursts that look like freezes.
  constexpr uint64_t MAX_STEPS_PER_SLICE = 2000;
  constexpr uint64_t STEPS_YIELD_CHUNK  = 256;
#endif
  ...
  while (true) {
    ...
    uint64_t elapsedUs = nowUs - lastUs;
    lastUs = nowUs;
    // Convert elapsed wall time into calibrated emulator steps.
    stepBudgetNum += elapsedUs * (uint64_t)EMU_CPU_FREQ_HZ * EMU_CPU_CYCLES_PER_STEP_DEN;
    uint64_t steps = stepBudgetNum / STEP_BUDGET_DEN;
    stepBudgetNum %= STEP_BUDGET_DEN;

    if (steps == 0) {
      // Prevent busy-spin when no full step is due yet.
      taskYIELD();
      continue;
    }

    if (steps > MAX_STEPS_PER_SLICE) {
      // Drop excess backlog to keep real-time responsiveness stable.
    ...
    for (uint64_t s = 0; s < steps; ++s) {
      i8086::step();
      m->tick();
      if ((s & (STEPS_YIELD_CHUNK - 1)) == (STEPS_YIELD_CHUNK - 1))
        taskYIELD();
    }
