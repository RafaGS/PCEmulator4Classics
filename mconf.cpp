...
// Mostrar también el nombre de plataforma (BIOS_OEM_NAME)
#include "emu_config.h"
#include "bios_platform.h"
void drawInfo(Canvas * canvas)
{
  canvas->setPenColor(Color::Yellow);
  canvas->drawText(93, 05, "   P C E m u l a t o r 4 C l a s s i c s   ");
  canvas->setPenColor(RGB888(0, 170, 0));
  canvas->drawText(93, 25, "  made into classics by minibots (RafaGS)  ");
  canvas->drawText(93, 35, "  >>>  more in minibots.wordpress.com  <<< ");
  canvas->drawText(93, 45, "based on PCEmulator by Fabrizio Di Vittorio");
  canvas->setPenColor(RGB888(255, 255, 255));
  canvas->drawText(93, 55, "Using ");
  canvas->drawText(150, 55, BIOS_OEM_NAME);
}
