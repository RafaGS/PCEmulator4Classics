...
boot_failed:
  ; Boot failed: invoke INT 18h (real IBM flow when boot from disk fails).
  int  18h
  hlt
  jmp  boot_failed
...
int10_set_cursor:
  ...
  ; BIOS Data Area cursor entry per page is a word at 0x0450 + page*2:
  ; low byte = column (DL), high byte = row (DH).
  mov   [si+curpos_x-bios_data], dx
